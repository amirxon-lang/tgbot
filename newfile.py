...
# Statistikani yangilash (faqat son topish o'yinida ball beriladi)
def update_stats(user_id: int, game_type: str):
    ...
    if game_type == "guess":  # Faqat son topish o'yinida ball beramiz
    ...
    else:  # Boshqa o'yinlar uchun faqat statistika
        user_stats[user_id]["total"]["won"] += 1
        user_stats[user_id][game_type]["won"] += 1
...
async def process_xo_move(callback: types.CallbackQuery):
    ...
    if result == "win":
        update_stats(callback.from_user.id, "xo")
...
async def process_tqq(callback: types.CallbackQuery):
    ...
    if user_choice == bot_choice:
        ...
        update_stats(callback.from_user.id, "tqq")
    else:
        result = "🤖 Bot yutdi!"
...
async def check_guess(message: types.Message):
    ...
    if guess < game["secret"]:
        ...
        update_stats(message.from_user.id, "guess")
        await message.reply(
            f"🎉 To'g'ri! {game['secret']} sonini {game['attempts']} urinishda topdingiz! +1 ball"
        )
...
async def process_word_guess(call: types.CallbackQuery):
    ...
    if game["word"][next_index] == letter:
        ...
        if "_" not in game["masked"]:
            update_stats(call.from_user.id, "word")
...
async def show_stats(message: types.Message):
    ...
    text = (
        ...
        f"⭐ Ballar: {stats['total']['points']}\n\n"
        f"❌⭕ X O: {stats['xo']['won']}/{stats['xo']['played']}\n"
        f"✊✌️✋ TQQ: {stats['tqq']['won']}/{stats['tqq']['played']}\n"
        f"🔢 Son topish: {stats['guess']['won']}/{stats['guess']['played']} (Ball: {stats['guess']['points']})\n"
...
async def show_top_players(message: types.Message):
    ...
    text = "🏆 Eng ko'p ball to'plaganlar:\n\n"
    for i, (user_id, stats) in enumerate(top_players, 1):
        ...
        text += f"{i}. {name} - {stats['total']['points']} ball\n"
